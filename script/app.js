...
function listab(e, t) {
    t = t || 0,
    $(e + " .tab-title").filter((function(e) {
        return e != t
    }
    )).addClass("tab-off"),
    $(".tab-content").eq(t).removeClass("none"),
    $(".tab-content").filter((function(e) {
        return e != t
    }
    )).hide(),
    $(e + " .tab-title").on("click", (function() {
        var t = $(this).data("tab-index");
        $(this).hasClass("tab-off") && ($(e + " .tab-title").addClass("tab-off"),
    ...
    setInterval((function() {
        t && (!function() {
            ...
            if (Math.abs(n - t) <= 1)
                return;
            t >= n ? ($(e).hide(),
            $("#mainpart").removeClass("menuside")) : t + $(window).height() < $(document).height() && ($(e).show(),
    ...
    if (a) {
        if ("object" === _typeof(a.errors))
            for (var s in a.errors)
                a.errors.hasOwnProperty(s) && a.errors[s].forEach((function(e) {
                    i += e + "\n"
                }
                ));
        else
...
$(".js-finished-series .slider").length && tns(Object.assign({
...
$("#sidenav-icon, .nav-user_icon, #noti-icon, .nav-has-submenu, #guest-menu").on("click", (function(e) {
    ...
    t.parents(".hidden-block").first().is(":visible") || ($(".active").not(t).removeClass("active"),
...
$(".nightmode-toggle").on("click", (function(e) {
    e.stopPropagation(),
    $(this).find(".toggle-icon").find("i").toggleClass("fa-toggle-off fa-toggle-on"),
    Cookies.get("night_mode") ? (Cookies.remove("night_mode"),
    $("#night-mode-css").remove(),
    $("html").removeClass("dark")) : (Cookies.set("night_mode", !0, {
        expires: 365
    }),
    $("html").addClass("dark"),
...
$("#bookmark_top").on("click", (function() {
    if ($(this).addClass("on"),
    !isLoggedIn)
        return alert("Bạn phải đăng nhập để sử dụng bookmark"),
        !1;
    ...
    bookmark_data.line_id > 0 && $.post("/action/chapter/bookmark", bookmark_data, (function(t) {
        "success" == t.status ? ($("ul#bookmarks_list").append('<li data-line="' + t.line_id + '"><span class="pos_bookmark">Đoạn thứ ' + t.line_id + '</span><small style="display: block">' + e + '</small><span data-item="' + t.bookmark_id + '" class="del_bookmark remove_bookmark"><i class="fas fa-times"></i></span></li>'),
        alert("Bạn đã lưu bookmark thành công đoạn thứ " + t.line_id)) : alert(t.message)
    }
    ), "json")
}
)),
...
$(".save_bookmark").on("click", (function() {
    ...
    $(window).width() > 979 && bookmark_data.line_id > 0 && $.post("/action/chapter/bookmark", bookmark_data, (function(e) {
        "success" == e.status ? ($("ul#bookmarks_list").append('<li data-line="' + e.line_id + '"><span class="pos_bookmark">Đoạn thứ ' + e.line_id + '</span><span data-item="' + e.bookmark_id + '" class="del_bookmark remove_bookmark"><i class="fas fa-times"></i></span></li>'),
        alert("Bạn đã lưu bookmark thành công đoạn thứ " + e.line_id)) : alert(e.message)
    }
    ), "json")
}
)),
...
$("ul#bookmarks_list").on("click", "span.remove_bookmark", (function() {
    ...
    $.post("/action/chapter/removebookmark", {
        _token: token,
...
Object.keys(readingObject).length)) {
    ...
    readingSeries.length > 24 && readingSeries.pop(),
...
$(".ln-comment-body").on("click", ".do-reply", (function() {
    ...
    if ($("#ln-comment-" + t).next().find("textarea.comment_reply").length)
        $(".reply-form").remove();
    else {
        $(".reply-form").remove();
        var o = t != n ? "@" + $("#ln-comment-" + t + " a.ln-username").text().trim() + ":&nbsp;" : "";
        $("#ln-comment-" + t).after($('<div class="ln-comment-reply reply-form"><div class="ln-comment-form"><textarea class="comment_reply"></textarea><div class="comment_toolkit clear"><input type="button" class="button submit_reply" value="Trả lời" data-parent="' + n + '"></div></div></div>')),
...
$(".ln-comment-body").on("click", ".paging_item, #refresh_comment", (function(e) {
    ...
    return $.post("/comment/ajax_paging", {
        ...
        if ("success" == e.status && "" != e.html) {
            $(".ln-comment-body").find(".ln-comment-group, .ln-comment-page").remove(),
...
$(".ln-comment-body").on("click", ".fetch_reply", (function(e) {
    var t = $(this);
    t.next().show(),
    $.post("/comment/fetch_reply", {
        _token: token,
        parent_id: t.data("parent"),
        offset: t.parent().find(".ln-comment-item").length,
        after: t.parent().find(".ln-comment-item").last().data("comment")
    }, (function(e) {
        "success" == e.status && "" != e.html ? (t.next().hide(),
        e.remaining > 0 ? t.text(e.fetchReplyText) : t.hide(),
        t.before(e.html),
        seeMoreButtons()) : "error" == e.status && alert(e.message)
    }
    ), "json")
}
)),
"undefined" != typeof series_id && series_id > 0 && (-1 == (Cookies.getJSON("mature_confirm") || []).indexOf(series_id) && $("#mature_modal").css("display", "block"),
$("button#mature_confirm").on("click", (function(e) {
    var t = Cookies.getJSON("mature_confirm") || [];
    -1 == t.indexOf(series_id) && (t.push(series_id),
...
"1" == $('meta[name="logged-in"]').attr("content") && !$("main.reading-page").length) {
    var handleNotiData = function(e) {
        e.notification_count > 0 ? ($("#noti-icon").find(".noti-unread").remove(),
        $("#noti-icon .icon-wrapper").append('<span class="noti-unread">' + e.notification_count + "</span>"),
        $("#noti-icon #noti-sidebar").find("#noti-content").prepend(e.html)) : $("#noti-icon").find(".noti-unread").remove(),
        e.seriesunread_count > 0 ? ($("#series-unread-icon").find(".noti-unread").remove(),
        $("#series-unread-icon .icon-wrapper").append('<span class="noti-unread">' + e.seriesunread_count + "</span>")) : $("#series-unread-icon").find(".noti-unread").remove(),
        e.pmunread_count > 0 ? ($(".at-user_avatar").addClass("icon-notify"),
        $(".at-user_list").addClass("icon-notify")) : ($(".at-user_avatar").removeClass("icon-notify"),
        $(".at-user_list").removeClass("icon-notify")),
        unreadCount = parseInt(e.total),
        setDocumentTitle(unreadCount > 0 ? "(" + unreadCount + ") " + pageTitle : pageTitle)
    }
    ...
    unreadCount > 0 && (document.title = "(" + unreadCount + ") " + document.title),
    ...
    function e() {
        setTimeout((function() {
            ...
            ((new Date).getTime() / 1e3 | 0) - (localStorage.getItem("ln_refresh_time") || 0) < refreshTime ? e() : (localStorage.setItem("ln_refresh_time", (new Date).getTime() / 1e3 | 0),
            $.post("/action/notification/fresh", {
                last: t
            }, (function(t) {
                "success" == t.status && (handleNotiData(t),
                localStorage.setItem("ln_crosstab", JSON.stringify(t))),
                e()
            }
            ), "json"))
        }
...
if ($(".spoiler_toggle").on("click", (function(e) {
    e.preventDefault();
    var t, n = $(this);
    return (t = n.parent().next()).is(":visible") ? (t.hide(),
    n.text("Click vào để hiển thị nội dung")) : (t.show(),
    n.text("Click vào để ẩn nội dung")),
    !1
}
)),
"undefined" != typeof series_id && series_id > 0 && $("span.star-evaluate-item").on("click", (function() {
    if ("1" == $('meta[name="logged-in"]').attr("content")) {
        ...
        alert("Bạn phải đăng nhập để đánh giá truyện")
}
)),
...
$(".gradual-mobile header").on("click", (function() {
    ...
    e.find("main").toggle(),
    $(this).find(".mobile-icon i").toggleClass("fa-chevron-down fa-chevron-up")
}
)),
$("#collect").on("click", (function(e) {
    $.post("/action/series/collect", {
        ...
        var t = $("#collect");
        "success" == e.status ? (e.collected ? alert("Bạn đã theo dõi truyện.") : alert("Bạn đã ngừng theo dõi truyện."),
        t.toggleClass("follow followed"),
        t.find("i").toggleClass("far fas")) : ($(".summary-content").css({
            maxHeight: "150px",
            overflow: "hidden"
        }),
...
$(".feature-section .summary-content").outerHeight() >= 100 && $(".feature-section .summary-more").removeClass("none"),
